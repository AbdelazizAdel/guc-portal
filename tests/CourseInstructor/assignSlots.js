...
describe('Testing Assigning Slots route',()=>{
    ...
    async ()=>{
        ...
        const output =  await(await request.patch('/academic-members/Nermeen2/slots/slot1').set('auth_token',token).send({instructorId:'Ashry19',courseID:'course1'})).text;
        console.log(output);
    ...
    ,it('A case where the member trying to assign is not a course instructor',async ()=>{
        const member = await createStaffMamber('Nermeen2','ashry@yahoo.com','ashduia','Nermeen Ashry','male',7000,3,'C6.305',6,[],undefined,false,[],true,'dep1');
        const response = await request.post('/login').send({email : member.email, password :'ashduia' });
        const token = response.headers.auth_token;
